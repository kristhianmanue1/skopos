...
| 19 | opencode al vigilante — cursor incremental para orígenes de filas (**ADR-013, 🔒 2026-09-06**, revoca §d de ADR-012 con medición completa: extracción completa 18.3 s vs delta por `time_created` 91.8 ms/24 h con índice cubriente existente; sellos de la delta byte-idénticos a la completa). `skopos-cursores/v2` aditivo, marca de agua con abridor previo (`_abridor_previo`), `procesar_base_filas` con la regla de congelamiento de ADR-011, fuente de filas en `watch` (cold start = pasada completa degradada, nunca pérdida; dedup en Mongo manda, ADR-005) | Cerrado | pendiente |
